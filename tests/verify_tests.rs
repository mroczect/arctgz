...
fn verify_checksum_mismatch() {
    with_temp_home(|home| {
        let project = home.join("bad_checksum");
        init(&project, false).unwrap();
        fs::write(project.join("include").join("data.bin"), b"original").unwrap();
        let mut config = load_config(&project).unwrap();
        config.include = vec!["data.bin".into()];
        save_config(&project, &config).unwrap();
        ...
        let fake_archive = home.join("fake.artgz");
        {
            ...
            let manifest = serde_json::json!({
                ...
                "files": {
                    "data.bin": { "size": 6, "sha512": "b3a8e0e1f9ab1bfe3a36f231f676f78bb30a519d2b21e6c530c0eee8ebb4a5d0" }
                }
            });
    ...
    with_temp_home(|home| {
        ...
        let archive = project.join("missing.artgz");
        {
            ...
            let manifest = serde_json::json!({
                ...
                "created": chrono::Utc::now().to_rfc3339(),
                "files": {
                    "ghost.txt": { "size": 0, "sha512": "cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e" }
                }
            });
            let manifest_bytes = serde_json::to_vec(&manifest).unwrap();
            let mut mheader = tar::Header::new_gnu();
