...
fn extract_restores_files() {
    with_temp_home(|home| {
        ...
        extract(&archive_path, &out_dir, false, None, None).unwrap();
...
fn extract_detects_checksum_mismatch() {
    with_temp_home(|home| {
        ...
        let res = extract(&archive_path, &out_dir, false, None, None);
...
fn extract_overwrite_with_force() {
    with_temp_home(|home| {
        ...
        let res = extract(&archive_path, &out_dir, false, None, None);
        assert!(res.is_err());
        extract(&archive_path, &out_dir, true, None, None).unwrap();
