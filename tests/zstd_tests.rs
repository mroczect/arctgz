...
fn compile_zstd_and_extract() {
    with_temp_home(|home| {
        ...
        verify(&archive, None, None).unwrap();

        let out_dir = home.join("out");
        extract(&archive, &out_dir, false, None, None).unwrap();
...
fn extract_zstd_checksum_mismatch() {
    with_temp_home(|home| {
        ...
        config.compression = Compression::Zstd;
        save_config(&project, &config).unwrap();
        let _archive = compile(&project, None, false, None, None).unwrap();
        ...
        let out_dir = home.join("out");
        let res = extract(&fake_archive, &out_dir, false, None, None);
