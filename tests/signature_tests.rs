...
fn sign_and_verify() {
    with_temp_home(|home| {
        ...
        let archive = compile(&project, None, false, Some(signing_key.as_bytes())).unwrap();

        verify(&archive, Some(verifying_key.as_bytes())).unwrap();

        let out = home.join("out");
        extract(&archive, &out, false, Some(verifying_key.as_bytes())).unwrap();
...
fn sign_and_verify_wrong_key_fails() {
    with_temp_home(|home| {
        ...
        let res = verify(&archive, Some(wrong_key.as_bytes()));
...
fn unsigned_archive_verify_with_public_key_fails() {
    with_temp_home(|home| {
        ...
        let res = verify(&archive, Some(vk.as_bytes()));
        assert!(matches!(res, Err(ArctgzError::SignatureError(_))));
    });
}
